...
{
    /// <summary>
    /// 管理面板（Form1_Regulator）
    /// 功能：监管员（Meruru/Utena）专用主界面
    /// 包含：用户信息卡片、魔女列表、数据管理功能、编辑公开描述功能
    /// </summary>
    public partial class Form1_Regulator : Form
    {
        #region 字段定义
        ...
        private int _currentIslandId = 0;  // 当前选中的岛屿ID（用于权限检查）
        ...
        private void InitializeContextMenu()
        {
            ...
            // 编辑公开描述
            var editDescItem = new ToolStripMenuItem("编辑公开描述");
        ...
        private void EditDescription_Click(object? sender, EventArgs e)
        {
            ...
            string currentDesc = drv["DescriptionPublic"]?.ToString() ?? "";

            // 检查权限：只能编辑本岛屿的魔女
            int witchIslandId = Convert.ToInt32(drv["IslandID"]);
            if (witchIslandId != _currentIslandId)
            {
                MessageBox.Show("您只能编辑本岛屿的魔女信息。", "权限不足", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // 打开编辑窗口
            var editForm = new WitchEditDescriptionForm(witchId, name, prisonerNo, currentDesc);
        ...
        private void ViewDetail_Click(object? sender, EventArgs e)
        {
            ...
            // 模拟双击事件
            var row = _grid.SelectedRows[0];
            Grid_CellDoubleClick(null, new DataGridViewCellEventArgs(0, row.Index));
        }
